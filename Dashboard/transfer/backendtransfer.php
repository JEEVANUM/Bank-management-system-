...
if (isset($_POST['transfer'])) {
	...
	if(empty($Account_number) || empty($amount_to_send) || empty($Account_name)){
		...
		$check_data = "SELECT  * FROM bnk WHERE accountnumber = '".$Account_number."' and username  = '".$Account_name."' ";
		...
		if (mysqli_num_rows($query_database) > 0) {
			if ($row_data = mysqli_fetch_assoc($query_database)) {
				...
				$update_amount = "UPDATE bnk SET totalamount= '".$_SESSION['totalamount']."' - '".$amount_to_send."'   where username = '".$_SESSION['username']."'  ";
				mysqli_query($conn,$update_amount);

				$update_totalamount  = "UPDATE bnk SET totalamount = '".$amount_to_send."' + '".$_SESSION['total']."'   where username= '".$Account_name."'  ";
				...
				exit();
						...
						header("location:\bankpro/Dashboard/error/transfererror.php");
			...
			echo "<h1> Account  number does not exist";
		}
