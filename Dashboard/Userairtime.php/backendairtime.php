...
if(isset($_POST['buy'])){
	...
	if(empty($phone_number) || empty($amount_to_buy)){
		...
		$check_data = "SELECT  * FROM bnk WHERE totalamount = '".$_SESSION['totalamount']."' and phonenumber = '".$phone_number."' ";
		...
		if (mysqli_num_rows($query_database) > 0 ) {
			if ($_SESSION['totalamount'] >= $amount_to_buy) {
				...
				$update_amount = "UPDATE bnk SET totalamount= '".$_SESSION['totalamount']."' - '".$amount_to_buy."'   where username = '".$_SESSION['username']."'  ";
				mysqli_query($conn,$update_amount);

				$update_totalamount  = "UPDATE bnk SET airtime = '".$amount_to_buy."' + '".$_SESSION['airtime']."'   where phonenumber = '".$phone_number."'  ";
				mysqli_query($conn,$update_totalamount);
				mysqli_commit($conn);
				header("location:\bankpro/Dashboard/success/airtimesuccess.php");
				exit();
			...
			echo "<h1> Phone number does not exist";
		}
