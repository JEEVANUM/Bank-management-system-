...
if(isset($_POST['submit'])){
	...
	if(empty($Phone_number)|| empty($Amount_to_buy) || empty($Pin)){
		...
		$check_data = "SELECT  * FROM bnk WHERE phonenumber = '".$Phone_number."' and pin_number  = '".$Pin."' ";
		...
		if (mysqli_num_rows($query_database) > 0) {
			if ($row_data = mysqli_fetch_assoc($query_database)) {
				...
				if ($_SESSION['total'] >= $Amount_to_buy) {
					...
					$update_amount = "UPDATE bnk SET totalamount= '".$_SESSION['total']."' - '".$Amount_to_buy."'   where pin_number= '".$Pin."'  ";
					mysqli_query($conn,$update_amount);

					$update_totalamount  = "UPDATE bnk SET airtime = '".$Amount_to_buy."' + '".$_SESSION['add_card']."'   where pin_number= '".$Pin."'  ";
					mysqli_query($conn,$update_totalamount);
					mysqli_commit($conn);
					header('Location:\bankpro/bankpage/gathomepage.php? u have bought airtime ');
					exit();
